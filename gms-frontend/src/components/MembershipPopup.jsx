...
const MembershipPopup = () => {
  ...
  let addMembership = async ()=>{
    try {
      let result = await axios.post(serverURL+"/api/plans/add-membership",inputField,{withCredentials:true})
      console.log(result)
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <div className="w-full h-full flex flex-col gap-[20px]">
      ...
      <div className="lower flex w-full justify-around px-[10px] py-[10px] mt-[20px]">
        ...
        />
        <button className="border-2 border-gray-400 px-[10px] py-[5px] rounded-lg cursor-pointer font-semibold hover:bg-linear-to-r from-indigo-500 to-pink-500 via-purple-500 hover:text-black hover:font-bold" onClick={()=>{
          addMembership()
        }}>
